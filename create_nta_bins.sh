...
do
    ...
    for LINES in $(seq 3 16)
    do

        rm -f .tmp

        OPT_EXE_BASE_NAME=${BENCH}_protean_nta_$((${LINES}-1))_amd
        ASM_OUTFILE=${OPT_EXE_BASE_NAME}.s

        for CURR_LINE in $(seq 3 ${LINES})
        do
            ADDR=$(sed ${CURR_LINE}'q;d' ${SCRIPTS_HOME}/${BENCH}_protean_amd.perinsntabw | awk '{print $1}')
            echo ${ADDR}':nta:0' >> .tmp
